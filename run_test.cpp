...
TEST_CASE("Test tree invariants", "[KDTree]"){
    ...
    auto tree = KDTree(std::move(dt), std::move(y), 0.1, 1e-7, 2);
...
TEST_CASE("Test min_rdist", "[KDTree]"){
    ...
    auto tree = KDTree(dt, y, 0.1, 1e-7, 2);
    std::cout << "\n-----------\n";
...
TEST_CASE("Test query", "[KDTree]"){
	...
	auto closest_index = res.front().index;
	auto index_res = tree.query_index(closest_index, nele);
	std::sort_heap(index_res.begin(), index_res.end());
	REQUIRE(index_res.front().index != closest_index);
	REQUIRE(index_res.front().rdistance < res.back().rdistance);
