...
class interpolation_storage {
  size_t nneighbours;
  std::vector<double> data;
  constexpr size_t get_size(size_t nneighbours) {
    return 3 * nneighbours + (nneighbours + nneighbours * nneighbours) / 2;
  }

public:
  explicit interpolation_storage(size_t nneighbours)
      : nneighbours(nneighbours), data(get_size(nneighbours)) {}

  void resize(size_t newnneighbours) {
    nneighbours = newnneighbours;
    data.resize(get_size(newnneighbours));
  }

  // Note: we don't make these properties because then we have to write all the
  // constructors.
  double *bstart() { return &data[0]; }
  double *wstart() { return &data[nneighbours]; }
  double *ystart() { return &data[2 * nneighbours]; }
  double *covmatstart() { return &data[3 * nneighbours]; }
};
...
double basic_rbf(double rdist, double lambda, double kernel_scale = 1) {
  return kernel_scale * kernel_scale *
         std::exp(-rdist / (2. * lambda * lambda));
}
...
struct KDTree {
  ...
  KDTree(Data2D datain, std::vector<double> const &y, double rbf_scale = 0.1,
         double noise_scale = 1e-7, size_t nneighbours = 20,
         double kernel_scale = 1, size_t leaf_size = default_leaf_size)
      : leaf_size(leaf_size), data(std::move(datain)),
        node_bounds(2, nnodes(), datain.nfeatures), rbf_scale(rbf_scale),
        noise_scale(noise_scale), kernel_scale(kernel_scale),
        nneighbours(nneighbours) {

	assert(data.nsamples >= nneighbours);
  ...
  double rbf(double rdist) { return basic_rbf(rdist, rbf_scale, kernel_scale); }
  ...
  interpolation_storage &compute_data_for_point_interpolation(
    ...
    // Need this because the line above is only called once for all instances
    internal_storage.resize(nneighbours);
  ...
  double get_variance(interpolation_storage &store) {
    return rbf(0) - std::inner_product(store.bstart(),
                                       store.bstart() + nneighbours,
                                       store.wstart(), 0.);
  }
  ...
  std::vector<DistanceIndex> query_index(size_t index, size_t k) {
    auto heap = std::vector<DistanceIndex>{};
    query_single_depthfirst_index(index, k, 0, 0., heap);
    return heap;
  }

  double interpolate_index(size_t index){
	  auto neigh = query_index(index, nneighbours);
	  auto &store = compute_data_for_point_interpolation(neigh);
	  return get_central_prediction(store);
  ...
  void query_single_depthfirst_index(size_t pt_index, size_t k, size_t inode,
                                     double minrdist,
                                     std::vector<DistanceIndex> &heap) {
    if (!heap.empty() && minrdist > heap.front().rdistance) {
      return;
    ...
    if (ndt.is_leaf) {
      for (size_t idx = ndt.start; idx < ndt.end; idx++) {
        // auto data_index = indexes[idx];
        auto data_index = idx;
        if (data_index != pt_index) {
          auto rdist = reduced_distance(data, data_index, pt_index);
          max_k_heap_push({rdist, data_index}, heap, k);
        }
      }
      ...
      auto child1 = 2 * inode + 1;
      auto child2 = 2 * inode + 2;
      auto d1 = min_rdist(child1, data.at(pt_index));
      auto d2 = min_rdist(child2, data.at(pt_index));
      if (d1 <= d2) {
        query_single_depthfirst_index(pt_index, k, child1, d1, heap);
        query_single_depthfirst_index(pt_index, k, child2, d2, heap);
      } else {
        query_single_depthfirst_index(pt_index, k, child2, d2, heap);
        query_single_depthfirst_index(pt_index, k, child1, d1, heap);
      }
